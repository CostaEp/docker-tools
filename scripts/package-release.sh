#!/usr/bin/env bash
# ==============================================================================
# MobyDock v2.2.0 — Offline Air-Gap Release Packaging Script
# ==============================================================================

set -e

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RELEASE_NAME="mobydock-release-v2.2.0"
DIST_DIR="${PROJECT_DIR}/dist"
TARGET_DIR="${DIST_DIR}/${RELEASE_NAME}"

echo "📦 Building Air-Gapped Release Package for MobyDock v2.2.0..."
echo "================================================================"

# Step 1: Ensure image is built
echo "1️⃣ Building Docker image docker-tools-mobydock:latest..."
docker build --platform linux/amd64 -t docker-tools-mobydock:latest -t docker-tools-mobydock:2.2.0 "${PROJECT_DIR}"
...
IMAGE_TAR="${TARGET_DIR}/mobydock-2.2.0-image.tar"
echo "3️⃣ Exporting offline Docker image tar archive (${IMAGE_TAR})..."
docker save docker-tools-mobydock:latest -o "${IMAGE_TAR}"
...
TARBALL_SHORT="${PROJECT_DIR}/mobydock-2.2.0.tar.gz"
